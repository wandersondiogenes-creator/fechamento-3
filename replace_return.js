...
const returnStartIndex = code.indexOf('  const selectedEmpData = selectedCompanyPanel');
...
// Find the start of the `wsResumo` block to locate the end of the return
const returnEndMarker = 'const wsResumo = XLSX.utils.aoa_to_sheet(resumoRows);';
const returnEndIndex = code.indexOf(returnEndMarker);
...
let beforeReturn = code.substring(0, returnStartIndex);
if (!beforeReturn.includes('const [selectedTab, setSelectedTab]')) {
  beforeReturn = beforeReturn.replace('const [selectedCompanyPanel, setSelectedCompanyPanel] = useState<string | null>(null);', 
    'const [selectedCompanyPanel, setSelectedCompanyPanel] = useState<string | null>(null);\n  const [selectedTab, setSelectedTab] = useState<\'departamentos\' | \'comparativo\' | \'resumo\'>(\'departamentos\');\n  const [selectedDeptForComp, setSelectedDeptForComp] = useState<string | null>(null);');
}
if (!beforeReturn.includes('getBrandLogo')) {
  beforeReturn = beforeReturn.replace('return (', `const getBrandLogo = (empName: string) => {
...
const newReturnStr = `  const selectedEmpData = selectedCompanyPanel && groupedByEmpresa[selectedCompanyPanel];
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2 bg-white rounded-xl p-1.5 border border-slate-200/60 shadow-sm">
          ...
          <button onClick={() => setFilterMode('pix_validation')} className={\`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all \${filterMode === 'pix_validation' ? 'bg-amber-50 text-amber-700 shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-amber-600'}\`}>
             <Clock className="w-3.5 h-3.5" /> Não Conciliado
          </button>
        </div>
          ...
          <div className="w-full lg:w-3/5 xl:w-2/3 flex flex-col gap-2.5">
            ...
            <div className="flex flex-col gap-3">
              {Object.entries(groupedByEmpresa).map(([empName, empData]) => {
                 ...
                 const logoUrl = getBrandLogo(empName);
                 
                 return (
                   <div 
                     key={empName}
                     onClick={() => { setSelectedCompanyPanel(isSelected ? null : empName); setSelectedTab('departamentos'); }}
                     className={\`group grid grid-cols-12 gap-4 items-center px-4 py-3.5 rounded-[1.25rem] border transition-all duration-300 cursor-pointer \${
                       isSelected 
                         ? 'bg-white ring-4 ring-blue-500/10 shadow-lg border-blue-200/60 scale-[1.01] z-10 relative' 
                         : hasDivergence 
                           ? 'bg-red-50/40 hover:bg-red-50/60 border-red-100 shadow-sm hover:shadow-md'
                           : isConciliada
                             ? 'bg-emerald-50/20 hover:bg-emerald-50/40 border-emerald-100 shadow-sm hover:shadow-md'
                   ...
                   >
                      <div className="col-span-5 lg:col-span-4 flex items-center gap-3 min-w-0">
                        {logoUrl ? (
                          <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shrink-0 shadow-inner p-1.5 border border-slate-100 overflow-hidden">
                             <img src={logoUrl} alt={empName} className="max-w-full max-h-full object-contain" />
                          </div>
                        ) : (
                      ...
                      <div className="col-span-3 lg:col-span-2 flex justify-center">
                        <button 
                          ...
                          className={\`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border shadow-sm transition-all hover:scale-105 active:scale-95 \${
                            hasDivergence ? 'bg-red-100/80 text-red-700 border-red-200/50 shadow-red-500/10' :
                            isConciliada ? 'bg-emerald-100/80 text-emerald-700 border-emerald-200/50 shadow-emerald-500/10' :
                            'bg-amber-100/80 text-amber-700 border-amber-200/50 shadow-amber-500/10'
                          }\`}
                        >
                          {hasDivergence ? <AlertCircle className="w-3.5 h-3.5" /> : isConciliada ? <CheckCircle2 className="w-3.5 h-3.5" /> : <Clock className="w-3.5 h-3.5" />}
                          <span className="hidden xl:inline">{hasDivergence ? 'Com Divergências' : isConciliada ? 'Conciliado' : 'Não Conciliado'}</span>
                        </button>
                      </div>
                 ...
                 <div className="bg-white/90 backdrop-blur-xl rounded-3xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.08)] overflow-hidden animate-in slide-in-from-right-8 fade-in duration-300">
                    <div className="p-6 pb-5 bg-gradient-to-b from-slate-50 to-white border-b border-slate-100">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                          {getBrandLogo(selectedCompanyPanel) ? (
                            <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shrink-0 shadow-lg p-2 border border-slate-100 overflow-hidden">
                               <img src={getBrandLogo(selectedCompanyPanel)!} alt={selectedCompanyPanel} className="max-w-full max-h-full object-contain" />
                            </div>
                          ) : (
                        ...
                        <button 
                          ...
                          className={\`flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold border hover:scale-105 active:scale-95 transition-all \${hasSelectedEmpDivergence ? 'bg-red-100 text-red-700 border-red-200' : isSelectedEmpConciliada ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-amber-100 text-amber-700 border-amber-200'}\`}
                        >
                          {hasSelectedEmpDivergence ? <AlertCircle className="w-3 h-3" /> : isSelectedEmpConciliada ? <CheckCircle2 className="w-3 h-3" /> : <Clock className="w-3 h-3" />}
                          {hasSelectedEmpDivergence ? 'Com Divergências' : isSelectedEmpConciliada ? 'Conciliado' : 'Não Conciliado'}
                        </button>
                      </div>
                    ...
                    <div className="px-6 py-2">
                      <div className="flex items-center gap-1 bg-slate-100/80 p-1 rounded-xl">
                        <button 
                          onClick={() => setSelectedTab('departamentos')}
                          className={\`flex-1 px-4 py-1.5 font-bold text-xs rounded-lg transition-all \${selectedTab === 'departamentos' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}\`}>
                          Departamentos
                        </button>
                        <button 
                          onClick={() => setSelectedTab('comparativo')}
                          className={\`flex-1 px-4 py-1.5 font-bold text-xs rounded-lg transition-all \${selectedTab === 'comparativo' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}\`}>
                          Comparativo
                        </button>
                        <button 
                          onClick={() => setSelectedTab('resumo')}
                          className={\`flex-1 px-4 py-1.5 font-bold text-xs rounded-lg transition-all \${selectedTab === 'resumo' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}\`}>
                          Resumo
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="px-4 pb-6 max-h-[500px] overflow-y-auto space-y-2">
                      {selectedTab === 'departamentos' && (
                        Object.entries(selectedEmpData.departamentos).map(([depTitle, dData]) => {
                          ...
                          return (
                            <div 
                              key={depTitle}
                              onClick={() => { setSelectedDeptForComp(depTitle); setSelectedTab('comparativo'); }}
                      ...
                      {selectedTab === 'comparativo' && (
                        <div className="flex flex-col gap-3">
                          {selectedDeptForComp && (
                            <div className="flex items-center justify-between bg-blue-50 text-blue-700 px-3 py-2 rounded-xl text-xs font-bold border border-blue-100">
                              <span>Dep: {selectedDeptForComp}</span>
                              <button onClick={() => setSelectedDeptForComp(null)} className="text-blue-500 hover:text-blue-700 underline text-[10px]">Ver todos</button>
                            </div>
                          )}
                          {Object.entries(selectedEmpData.departamentos)
                            .filter(([dName]) => !selectedDeptForComp || dName === selectedDeptForComp)
                            .map(([dName, dData]) => {
                              // Match items by exact absolute value
                              const dealerItems = dData.items.filter(i => i.origem === 'dealer' || i.banco?.toUpperCase() === 'DEALER');
                              const sitefItems = dData.items.filter(i => i.origem !== 'dealer' && i.banco?.toUpperCase() !== 'DEALER');
                              
                              let pairs: {dealer?: any, sitef?: any}[] = [];
                              let remainingSitef = [...sitefItems];
                              
                              dealerItems.forEach(dItem => {
                                const matchIdx = remainingSitef.findIndex(sItem => Math.abs(sItem.valor || 0) === Math.abs(dItem.valor || 0));
                                if (matchIdx !== -1) {
                                  pairs.push({ dealer: dItem, sitef: remainingSitef[matchIdx] });
                                  remainingSitef.splice(matchIdx, 1);
                                } else {
                                  pairs.push({ dealer: dItem });
                                }
                              });
                              remainingSitef.forEach(sItem => pairs.push({ sitef: sItem }));
                              
                              return pairs.map((pair, idx) => {
                                const isMatched = pair.dealer && pair.sitef;
                                const hasDivergence = !isMatched;
                                return (
                                  <div key={\`comp-\${dName}-\${idx}\`} className={\`flex flex-col gap-2 p-3 rounded-2xl border text-xs \${hasDivergence ? 'bg-red-50/50 border-red-100' : 'bg-emerald-50/50 border-emerald-100'}\`}>
                                     {/* Row Dealer */}
                                     {pair.dealer && (
                                       <div className="flex justify-between items-center bg-white p-2 rounded-xl border border-slate-100 shadow-sm">
                                         <div className="flex flex-col min-w-0">
                                           <span className="font-bold text-slate-800 text-[10px] uppercase text-blue-600">DEALER</span>
                                           <span className="text-slate-600 truncate">{pair.dealer.descricao || 'Lançamento'}</span>
                                         </div>
                                         <span className="font-mono font-bold text-slate-700 whitespace-nowrap ml-2">{formatBRL(pair.dealer.valor)}</span>
                                       </div>
                                     )}
                                     
                                     {/* Link / Gap */}
                                     <div className="flex justify-center -my-2 relative z-10">
                                       <div className={\`w-6 h-6 rounded-full flex items-center justify-center border-2 border-white shadow-sm \${isMatched ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white'}\`}>
                                         {isMatched ? <Check className="w-3 h-3 stroke-[3]" /> : <AlertCircle className="w-3 h-3" />}
                                       </div>
                                     </div>
                                     ...
                                     {pair.sitef && (
                                       <div className="flex justify-between items-center bg-white p-2 rounded-xl border border-slate-100 shadow-sm">
                                         <div className="flex flex-col min-w-0">
                                           <span className="font-bold text-slate-800 text-[10px] uppercase text-indigo-600">CTF / SITEF</span>
                                           <span className="text-slate-600 truncate">{pair.sitef.descricao || 'Lançamento'}</span>
                                         </div>
                                         <span className="font-mono font-bold text-slate-700 whitespace-nowrap ml-2">{formatBRL(pair.sitef.valor)}</span>
                                       </div>
                                     )}
                                  </div>
                                );
                          ...
                          {Object.keys(selectedEmpData.departamentos).length > 0 && selectedTab === 'departamentos' && (
                      ...
                      {selectedTab === 'resumo' && (
                        <div className="p-4 text-center text-slate-500 text-xs font-semibold bg-white rounded-2xl border border-slate-100 shadow-sm">
                          Resumo da empresa {selectedCompanyPanel}
                        </div>
                      )}
                    </div>
                 </div>
...
fs.writeFileSync(file, beforeReturn + newReturnStr);
console.log("Successfully replaced return code");
